...
![Spring Boot](https://img.shields.io/badge/Spring_Boot-4.0-brightgreen?logo=spring)
![React](https://img.shields.io/badge/React-19-61DAFB?logo=react)
![Deploy](https://img.shields.io/badge/deploy-GitHub_Actions-2088FF?logo=githubactions)

서울시 **기후동행카드** 사용 가능 버스 노선을 지도에서 실시간으로 확인하고,  
출발지-도착지 경로 탐색 시 기후동행카드 사용 가능 여부를 판단해주는 웹 서비스입니다.
...
| 기능 | 설명 |
|------|------|
| 주변 정류장 조회 | GPS 기준 반경 500m 이내 버스 정류장 표시 |
| 기후동행 노선 필터 | 기후동행카드 사용 가능 버스만 필터링 |
| 실시간 버스 도착 정보 | 정류장 선택 시 도착 예정 시간 표시 (30초 자동 갱신) |
| 경로 탐색 | 대중교통 경로 탐색 + 기후동행카드 사용 가능 여부 판단 |
| 실시간 내 위치 | GPS 기반 현재 위치 실시간 갱신 |
...
- **Caffeine Cache** — 기후동행 노선 목록 1시간 캐싱
...
### 인프라
- **Docker Compose** — postgres / backend / frontend 3개 컨테이너
- **nginx** — FE 정적 파일 서빙 + SPA fallback
- **GitHub Actions** — main push 시 서버 자동 배포
...
FE (`Climate-Bus-Map-FE/.env.local`):
```env
...
| `GET` | `/api/v1/stations/{stationId}/arrivals` | 실시간 버스 도착 정보 |
